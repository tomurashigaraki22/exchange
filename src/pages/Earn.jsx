  ...
  const handleClaim = () => {
    if (shares === 20) {
      if (parseFloat(amount) == 0) {
        toast.error(
          "You must have an existing plan before getting referral bonus",
          { theme: "dark" }
        );
      } else {
  ...
  const add_to_balance = async (amt) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/referral_claim`, {
        userEmail: email,
        newBalance: parseFloat(amt.toFixed(2)) || 0,
      });
      ...
      const err = error.response.data.message || "Error adding to balance";
