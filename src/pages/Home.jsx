...
const Home = ({ socket }) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        const response = await axios.post(`${BASE_URL}/get_balance`, {
          email,
        });
  ...
  useEffect(() => {
    socket.on("balanceUpdated", (data) => {
      console.log(data);
      localStorage.setItem("balance", data.newBalanceToNumber);
      setBalance(data.newBalanceToNumber);
    });
    socket.on("planUpdated", ({ updatedPlan, updatedAmount }) => {
      setPlan(updatedPlan);
      ...
      <div className="bg-darkGreen py-3 px-4 pb-6">
        ...
        <div className="flex flex-col items-center border border-white bg-white p-4 rounded-lg mt-5">
          ...
          <div className="flex flex-row items-center justify-between w-full space-x-3 mt-4">
            {[
              {
                icon: <BsArrowUp color="white" size={21} />,
                text: "Deposit",
                action: openModal,
              },
              {
                icon: <BsArrowDown color="white" size={21} />,
                text: "Withdraw",
              },
              { icon: <CgCreditCard color="white" size={21} />, text: "Cards" },
              {
                icon: <LuArrowUpDown color="white" size={21} />,
                text: "Exchange",
              },
            ].map((item, index) => (
              <div
      ...
      <div className="bg-gray-900 w-full py-6 px-4 flex-1">
        <div className="flex flex-row items-center justify-between space-x-3">
          <div className="border-2 border-white bg-white w-1/2 h-20 rounded-lg p-2 pt-3">
            ...
            <p className="text-darkGreen text-xl pt-3">
              ${balance ? balance : "null"}
            </p>
          </div>
          <div className="border-2 border-white bg-white w-1/2 h-20 rounded-lg p-2 pt-3">
            <p className="text-gray-500 text-[12px]">Current Investment Plan</p>
            <p className="text-blue-900 text-xl pt-3">
              {plan !== "undefined" ? plan : "None"} : {amount}
            </p>
          </div>
        </div>
        <div className="flex flex-row items-center justify-between space-x-3 mt-4">
          <div className="border-2 border-white bg-white w-1/2 h-20 rounded-lg p-2">
            <p className="text-gray-500 text-[12px]">
              Amount Made From Website (/m)
            </p>
            <p className="text-darkGreen text-xl">$30,439.23</p>
          </div>
          <div className="border-2 border-white bg-white w-1/2 h-20 rounded-lg p-2 pt-3">
            <p className="text-gray-500 text-[12px]">
              Highest Investor Balance
            </p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-800 flex justify-center items-center max-h-70 p-10">
          ...
          <div className="space-y-5">
            ...
            <div
              className="flex flex-row items-center space-x-2 justify-center rounded-lg text-center py-3 bg-green-900 text-white group hover:bg-green-800 transition duration-300 ease-in-out"
              onClick={sendSMS}
            >
